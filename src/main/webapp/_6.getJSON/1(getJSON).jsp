 ...
 5. 서버로부터 get방식의 요청을 하고, 응답을 JSON 형식으로 받기 위해서
    ...
    (2) 서버로 요청이 성공적으로 완료되면, 두번째 매개변수로 기술한 콜백함수가 호출됩니다.
       형식) function(data, textStatus)
           ①콜백함수의 첫번째 매개변수인 data는 서버에서 돌려받은
              JSON 객체 형식의 데이터입니다.
           ②두번째 매개변수인 textStatus는 성공일 때 "success" 라는 문자열이 전달됩니다.
           
    (3) 자바스크립트의 JSON객체는 JSON데이터를 자바스크립트 객체로 변경할 수 있습니다.
       또한 자바스크립트 객체를 문자열로 변환하기도 합니다.
       ① JSON.stringify()메서드 : 자바스크립트 객체를 JSON형식의 문자열로 변환됩니다.
          이를 이용해서 자바스크립트 객체를 브라우저로부터 다른 애플리케이션으로 전송할 수 있습니다.
       ② JSON.parse()메서드 : JSON데이터를 부라우저가 사용할 수 있는 자바스크립트 객체로 변환합니다.
 */
$(function(){
   $("button").click(function(){ //버튼을 클릭하면
      $.getJSON('item.json', function(data, textStatus){
         ...
         if(data.length>0){
            var output = "<table class=table><thead><tr><td>id</td><td>name</td>"
